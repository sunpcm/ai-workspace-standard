...
- Current version: v1.1.0 local candidate; v1.0.0 published on 2026-08-08
...
# Check the documentation language boundary
...
- `python3 scripts/aews_validate.py . --mode template` reports no failures or
  warnings,
- validator regression tests pass, including the documentation language
  boundary check.
...
- A translation such as `README.zh-CN.md` can drift from its English original.
  No automated check compares their meaning; keep translations in the same
  change as the original or mark them stale.
